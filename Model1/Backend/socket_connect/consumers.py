...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    def generate_opensees_script(self, json_data):
        ...
        (Omitting any interactive plotting; builds, analyzes, prints results.)
        """

        lines = []

        # --- Change 1: Stability Check (in the converter, before emitting any OpenSees commands) ---
        total_restraints = sum(bc.get("xd", 0) + bc.get("yd", 0)
                            for bc in json_data.get("supports", []))
        n_elements = len(json_data.get("elements", []))
        n_nodes = len(json_data.get("nodes", []))
        if n_elements + total_restraints < 2 * n_nodes:
            lines.append(
                f"# WARNING: Model may be unstable! "
                f"(elements + restraints = {n_elements + total_restraints} < 2*{n_nodes})"
            )
        ...
        element_info = []  # store (etag, ni, nj) to use later for force extraction
        for el in json_data.get("elements", []):
            ...
            if etype.lower() == "truss":
                ...
                element_info.append((etag, start, end))
            else:
        ...
        # 9. Retrieve Results (Displacements & Reactions)
        ...
        # 11. Change 2: Retrieve Element Forces via basicForce() & Build a DataFrame
        lines.append("# --- Compute MEMBER FORCES & Classify Tension/Compression ---")

        # Collect the element‐tag, start‐node, end‐node triples into a Python list literal
        # so that the generated script can loop over them.
        elem_list_literal = "[" + ", ".join(
            f"({etag}, {ni}, {nj})" for etag, ni, nj in element_info
        ) + "]"
        lines.append(f"element_data = {elem_list_literal}   # list of (etag, node_i, node_j)")

        lines.append("forces = []")
        lines.append("for etag, ni, nj in element_data:")
        lines.append("    # basicForce(etag) returns a Python list; index 0 is the axial force")
        lines.append("    f_axial = basicForce(etag)[0]")
        lines.append("    state = 'Tension' if f_axial > 0 else 'Compression'")
        lines.append("    forces.append([etag, ni, nj, f_axial, state])")
        lines.append("")
        lines.append("df_forces = pd.DataFrame(forces, columns=[""'element', 'node_i', 'node_j', 'axial_force', 'state'])")
        lines.append("print('=== MEMBER FORCES ===')")
        lines.append("print(df_forces.to_string(index=False))")
        lines.append("")
