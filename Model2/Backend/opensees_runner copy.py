...
class OpenSeesRunner:
    ...
    def execute_command(self, cmd_dict):
        ...
        # Capture anything OpenSees prints
        ...
        try:
            ...
            err_text = stderr_buf.getvalue().strip() or str(e)
            self.results["status"] = "command_error"
        ...
        std_err = stderr_buf.getvalue().strip()
        std_out = stdout_buf.getvalue().strip()
        if std_err:
            if std_err.lower().startswith("warning"):
                self.results["warnings"].append(std_err)
            else:
    ...
    def run_analysis_sequence(self):
        ...
        for step in self.data["analysis_sequence"]:
            try:
                ...
                # if analyze fails (non-zero), mark error but do not crash
                if step["command"] == "analyze" and res not in (0, None):
                ...
                if not step.get("optional", False):
                    self.results["status"] = "analysis_exception"
                    break
